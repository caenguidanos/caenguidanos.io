...
export function remarkShikiComposeHighlighterCache(nextHighlighter: Highlighter): HighlighterWithCache {
...
export function remarkShikiCreateHighlighter({ theme }: RemarkShikiCreateHighlighter): Promise<HighlighterWithCache> {
   return new Promise<HighlighterWithCache>((resolve, reject) => {
      getHighlighter({ theme: theme as string })
         .then((nextHighlighter) => {
            try {
               const highlighterWithCacheID: HighlighterWithCache = remarkShikiComposeHighlighterCache(nextHighlighter);
...
export function remarkShikiCacheHighlighter({ theme }: RemarkShikiCacheHighlighter): () => Promise<Highlighter> {
